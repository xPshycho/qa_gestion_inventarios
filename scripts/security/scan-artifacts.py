#!/usr/bin/env python3
"""Fail closed when CI evidence contains configured or encoded secrets."""
...
def account_expansion(size: int) -> None:
    ...
    if expanded_bytes > MAX_EXPANDED_BYTES:
        raise UnsafeArtifact("encoded artifact exceeds the safe expansion limit")
...
def inspect_payload(data: bytes, label: str, depth: int = 0) -> None:
    ...
    if len(data) > MAX_INPUT_BYTES:
        raise UnsafeArtifact(f"{label}: file or payload exceeds the safe scan limit")
    if any(secret in data for secret in SECRETS):
        raise UnsafeArtifact(f"{label}: configured secret detected")
    if any(encoded in data for encoded in SECRET_ENCODINGS):
        raise UnsafeArtifact(f"{label}: base64-encoded secret detected")
    if any(encoded in data for encoded in BASIC_AUTH_ENCODINGS):
        raise UnsafeArtifact(f"{label}: Basic authentication value detected")
    if JWT_PATTERN.search(data):
        raise UnsafeArtifact(f"{label}: JWT-like value detected")
            ...
            for member in archive.infolist():
                ...
                if member.file_size > MAX_INPUT_BYTES:
                    raise UnsafeArtifact(
                        f"{label}: archive member exceeds the safe limit"
                    )
...
def main() -> int:
    if len(sys.argv) != 2:
        print("usage: scan-artifacts.py ARTIFACT_PATH", file=sys.stderr)
        return 2

    artifact_path = Path(sys.argv[1]).resolve()
    if not artifact_path.exists() or artifact_path.is_symlink():
        print("artifact path does not exist or is a symbolic link", file=sys.stderr)
        return 1
    artifact_files = (
        (artifact_path,)
        if artifact_path.is_file()
        else tuple(sorted(artifact_path.rglob("*")))
    )

    try:
        for artifact_file in artifact_files:
            if artifact_file.is_symlink():
                raise UnsafeArtifact("symbolic links are not allowed in artifacts")
            if not artifact_file.is_file():
                continue
            if artifact_file.stat().st_size > MAX_INPUT_BYTES:
                raise UnsafeArtifact(
                    f"{artifact_file.name}: file exceeds the safe scan limit"
                )
            inspect_payload(artifact_file.read_bytes(), artifact_file.name)
    except (OSError, RuntimeError, UnsafeArtifact, zipfile.BadZipFile) as error:
        print(f"artifact safety scan failed: {error}", file=sys.stderr)
        return 1

    print(
        "Recursive artifact scan passed "
