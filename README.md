...
| Alertmanager | Pendiente de issue posterior |

Grafana y Prometheus se levantan junto al stack local con Docker Compose. Grafana queda disponible
en `http://localhost:3000` con credenciales demo `admin` / `cambiar`, configurables en `.env` con
`GRAFANA_ADMIN_USER` y `GRAFANA_ADMIN_PASSWORD`. Prometheus queda disponible en
`http://localhost:9090` y scrapea `backend:8080/actuator/prometheus`.

La guia de verificacion del dashboard operativo esta en `docs/observability/grafana.md`.
...
- `docs/observability/grafana.md`: Prometheus, Grafana, dashboard operativo y pasos de verificacion.
